  ...
  <main>
    <section class="projects-header">
      <div id="header-text" class="w-full flex flex-col justify-center p-4 py-52 pt-60 relative">
        <h1 id="header-title" class="tracking-tight text-5xl uppercase overflow-hidden mb-4"><span v-for="(word, index) in titleWords" :key="index" class="title-words inline-block"><span v-for="(letter, index2) in Array.from(word)" :key="index2" class="">{{ letter }}</span>&nbsp;</span><span class="inline-block text-base align-top ml-1">{{projects.length}}</span></h1>
    ...
    <section class="content">
      <div v-if="projects.length > 0" class="grid grid-cols-12 px-4 pt-12 py-24 gap-4 gap-y-12 lg:gap-y-40 items-end">
